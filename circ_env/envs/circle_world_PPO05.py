...
class CircleEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        FK = np.zeros(pos_p.shape)
        if dist > 0:
            direction = self.unit_vector(pos_a,pos_p)#(pos_a - pos_p) / np.linalg.norm(pos_a - pos_p)
            FK = K*dist*direction
        vel_p = self.get_puck_velocity()
        FB = -B*vel_p
        F = np.zeros(pos_p.shape)
        acc = np.zeros(pos_p.shape)
        F = FK+FB
        acc = F / m
